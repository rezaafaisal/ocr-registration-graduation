...
@section('title', __('edit operator'))
@section('head')
    @vite('resources/js/admin/user/operator/edit.js')
@endsection
@section('body')
@endsection
...
@section('content')
    <div class="grid gap-4">
        ...
        <form
            ...
            action="{{ route('admin.user.operator.update', ['operator' => $data]) }}" method="post"
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <select id="department" name="department"
                        ...
                        @foreach (App\Models\Operator::$list_department as $key => $value)
                            <option @selected($key == (old('department') ?? $data->department)) value="{{ $key }}" class="capitalize">
                                {{ $value }}</option>
                        @endforeach
                    </select>
                    @error('department')
