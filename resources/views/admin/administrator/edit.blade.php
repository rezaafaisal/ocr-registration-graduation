...
@section('content')
    <div class="grid gap-4">
        ...
        <form
            ...
            action="{{ route('admin.user.administrator.update', ['administrator' => $data]) }}" method="post"
            ...
            <div class="grid gap-4">
                {{-- <div class="flex items-end gap-4">
                    <div
                        ...
                        <img id="photo_preview" @class(['hidden' => !$data->photo]) src="{{ $data->photo_url }}" alt="photo">
                    </div>
                ...
                {{-- <div class="flex flex-col gap-2">
                    ...
                    <select id="role" name="role"
                        ...
                        @foreach (App\Models\AdministratorRole::to_array() as $key => $value)
                            <option @selected($key == (old('role') ?? $data->role)) value="{{ $key }}" class="capitalize">
                                {{ $value }}</option>
                        @endforeach
                    </select>
                    @error('role')
