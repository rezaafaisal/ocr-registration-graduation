@extends('layouts.student.panel', ['content_card' => false])
    ...
    <div class="grid grid-flow-col grid-cols-2 gap-4">
        <div class="h-fit p-4 bg-white border rounded-lg shadow-md sm:p-8 dark:bg-gray-800 dark:border-gray-700">
            <div class="flex items-center justify-between mb-4">
                ...
                <form class="contents"
                    action="{{ route('notification.mark_all', ['guard' => 'student']) }}" method="post">
                    @csrf
                    @method('PATCH')
                        ...
                        <li class="py-3 sm:py-4">
                            <div class="flex items-center space-x-4">
                                ...
                                <a class="text-sm font-medium text-blue-600 hover:underline dark:text-blue-500"
                                    href="{{ route('notification.read', ['guard' => 'student', 'id' => $notification]) }}">
                                    Read
                                </a>
                            </div>
                        </li>
        ...
        <div class="h-fit p-4 bg-white border rounded-lg shadow-md sm:p-8 dark:bg-gray-800 dark:border-gray-700">
            <div class="flex items-center justify-between mb-4">
                ...
                <form class="contents"
                    action="{{ route('notification.delete_all', ['guard' => 'student']) }}" method="post">
                    @csrf
                    @method('DELETE')
                    ...
                    @forelse ($user->readnotifications as $notification)
                        <li class="py-3 sm:py-4">
                            <div class="flex items-center space-x-4">
                                ...
                                <form class="contents"
                                    action="{{ route('notification.delete', ['guard' => 'student', 'id' => $notification]) }}"
                                    method="post">
                                    @csrf
                                    @method('DELETE')
                                    <button
