@props([
    ...
    'delete' => function ($item) {
        return route('resources.archive.quota.delete', ['quota' => $item]);
    },
    'delete_any' => route('resources.archive.quota.delete_any'),
])
...
<div class="grid gap-4">
    <div class="flex gap-2 items-center">
        ...
        @endcan
        @can('deleteAny', App\Models\ArchiveQuota::class)
            <form class="contents" id="delete_any" action="{{ $delete_any }}" method="post"
                            ...
                            @switch($key)
                                @case('ipk')
                                    <div class="flex flex-col gap-1">
                                        ...
                                        <input type="number" step=".1" max="4" min="0"
                                            id="f_ipk" name="f_ipk" value="{{ request()->query('f_' . $key) }}"
                                ...
                                @case('registrars')
                                    <td class="py-4 px-6 text-gray-900 dark:text-white whitespace-nowrap">
                                        <div class="grid place-content-center">
                                            <a href="{{ route('admin.archive.quota.registrar.index', ['quota' => $item]) }}"
                    ...
                    <td
                        ...
                        @can('delete', $item)
                            <form id="item-delete-{{ $item->id }}" class="contents"
                                action="{{ $delete($item) }}" method="post">
                                @csrf
                                @method('DELETE')
